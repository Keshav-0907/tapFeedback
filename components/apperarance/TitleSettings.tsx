...
const TitleSettings = ({ popupStyles, setPopupStyles }:TitleSettingsProps) => {
  return (
    <Card className="p-4">
      <div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-1 items-center">
              <Slider
                value={[parseInt(popupStyles?.titleSize || '16')]}
                min={5}
                max={50}
                step={1}
                onValueChange={([val]) => {
                  setPopupStyles({ ...popupStyles!, titleSize: `${val}` });
                }}
                className="w-32"
              />
              <span className="text-sm">{popupStyles?.titleSize || '16'}px</span>
            </div>
          </div>
        </div>
      </div>
